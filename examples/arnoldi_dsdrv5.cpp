// C++ port of EXAMPLES/SYM/dsdrv5.f using arnoldi::Arnoldi<Kind::Sym, double>.
//
// Generalized symmetric eigenvalue problem in buckling mode (Mode 4):
//
//     K*x = lambda*KG*x,   OP = inv(K - sigma*KG)*K,   B = K
//
// K is the 1-D FE stiffness matrix (Laplacian) on [0,1].
// KG is the associated FE mass matrix (piecewise linear elements).
//
// In buckling mode, WHICH="LM" finds eigenvalues of OP with largest magnitude,
// which correspond to buckling loads closest to sigma.
//
// The shift SIGMA must be non-zero for buckling mode.
...
// K*x (stiffness matrix = FE Laplacian)
...
int main() {
    ...
    // Factor (K - sigma*KG) as tridiagonal
    ...
    if (int info = arnoldi_examples::lapackx::gttrf<double>(
        ...
        std::printf(" Error with _gttrf in _SDRV5.  info=%d\n", info);
        return 1;
    }

    arnoldi::Arnoldi<arnoldi::Kind::Sym, double> solver("G", n, "LM", nev, ncv);
    solver.tol(0.0).maxiter(300).mode(4).ishift(1);

    // OP: y = inv(K - sigma*KG) * K*x
    ...
    // Residuals: ||K*x - lambda*KG*x|| / |lambda|
    ...
    std::printf(" What portion of the spectrum:          LM (buckling, sigma=%.3f)\n", sigma);
